...
Graph graphIn() {
  ...
  fscanf(f, "%d", &n);
  Graph g(n);
  while(fscanf(f, "%d%d%lf", &v1, &v2, &weight) != EOF) {
...
void fileOut(vector<Edge> mst_edge, vector<Path> shortestPath) {
  FILE *f;
  double sum_weight = 0;

  if (NULL == (f = fopen("t2_out.txt", "w"))) {
    printf("Fail to open the file");
    exit(EXIT_FAILURE);
  }

  fprintf(f, "Edges in MST(vertex1, vertex2, weight):\n");
  for(int i = 0; i < mst_edge.size(); ++i) {
    fprintf(f, "(%d, %d, %.2f)", mst_edge[i].tail, mst_edge[i].head, mst_edge[i].weight);
    sum_weight += mst_edge[i].weight;
    fprintf(f, "\n");
  }
  fprintf(f, "Sum of weight: %.2f\n\n", sum_weight);

  fprintf(f, "Shortest path from vertex1\n");
  for(int i = 1; i < shortestPath.size(); ++i) {
    fprintf(f, "1 -> %d: %.2f, path:", i + 1, shortestPath[i].length);
    for(int j = 0; j < shortestPath[i].path.size(); ++j)
      fprintf(f, "%d ", shortestPath[i].path[j]);
...
int main() {
  ...
  //output to file "t2_out.txt"
  fileOut(mst_edge, shortestPath);
}
